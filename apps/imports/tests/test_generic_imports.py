...
class GenericContactImportTests(TestCase):
    ...
    def test_generic_contact_import_creates_new_contacts(self):
        ...
        batch = import_generic_contacts(
            file_bytes, 'contacts.csv', self.user, self.user, match_by='email',
        )
        ...
        self.assertTrue(
            Contact.objects.filter(
                owner=self.user, email='john@example.com',
            ).exists()
        )
        self.assertTrue(
            Contact.objects.filter(
                owner=self.user, email='jane@example.com',
            ).exists()
        )
    ...
    def test_generic_contact_import_updates_existing_by_email(self):
        ...
        # Verify phone was merged
        contact = Contact.objects.get(owner=self.user, email='john@example.com')
    ...
    def test_generic_contact_import_match_by_name(self):
        ...
        batch = import_generic_contacts(
            file_bytes, 'contacts.csv', self.user, self.user, match_by='name',
        )
...
class GenericDonationImportTests(TestCase):
    ...
    def test_generic_donation_import_creates_gifts(self):
        ...
        self.assertEqual(
            gifts.order_by('gift_date').first().amount_cents, 10000,
        )
