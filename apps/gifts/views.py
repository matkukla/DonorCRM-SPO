...
class GiftListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        qs = Gift.objects.select_related(
            "donor_contact",
            "donor_contact__owner",
            "fund",
            "recurring_gift",
        ).all()
...
class GiftDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        ...
        if user.role == "coach":
            return Gift.objects.none()
        qs = (
            Gift.objects.select_related(
                "donor_contact",
                "donor_contact__owner",
                "fund",
                "recurring_gift",
            )
            .prefetch_related("credits__solicitor")
            .all()
        )
        visible = get_visible_user_ids(user, request=self.request)
        qs = qs.filter(donor_contact__owner_id__in=visible)
...
class RecurringGiftListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        qs = RecurringGift.objects.select_related(
