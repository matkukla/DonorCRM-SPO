...
@extend_schema_view(
    get=extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                name="needs_thank_you", description="Filter by thank-you status", type=bool
            ),
...
class ContactListCreateView(generics.ListCreateAPIView):
    ...
    # 'email' is encrypted at rest — DRF's SearchFilter would issue
    # __icontains against the ciphertext column and never match. Use the
...
class ContactDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = Contact.objects.filter(owner_id__in=visible)
        return (
            queryset.filter(is_merged=False)
...
class ContactGiftsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        visible = get_visible_user_ids(user, request=self.request)
        return (
            Gift.objects.filter(donor_contact_id=contact_id, donor_contact__owner_id__in=visible)
            .select_related("donor_contact", "donor_contact__owner", "fund", "recurring_gift")
            .order_by("-gift_date")
        )
...
class ContactRecurringGiftsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .select_related("donor_contact", "donor_contact__owner", "fund")
            .order_by("-start_date")
        )
...
class ContactEmailsView(APIView):
    ...
    def get(self, request):
        ...
        # email is encrypted; partial-match search is no longer supported,
        # but a full-email query still matches via email_hash equality.
        search = request.query_params.get("search", "").strip()
        if search:
            from apps.core.blind_index import lookup_hashes

            email_hashes = lookup_hashes(search)
            email_q = Q(email_hash__in=email_hashes) if email_hashes else Q(pk__in=[])
...
class ContactSearchView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if query:
            ...
            email_hashes = lookup_hashes(query)
            phone_hashes = lookup_hashes(normalize_phone(query))
            # email + phone are encrypted: substring search is gone, but a
            # full-value query still matches via the blind indexes.
            email_q = Q(email_hash__in=email_hashes) if email_hashes else Q(pk__in=[])
            phone_q = (
...
class ContactJournalsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        memberships = (
            JournalContact.objects.filter(contact_id=contact_id)
            .select_related("journal")  # ForeignKey - single JOIN
            .prefetch_related(
                Prefetch(
                    "stage_events",
                ...
                Prefetch(
                    "decisions", queryset=Decision.objects.all(), to_attr="prefetched_decisions"
                ),
            )
            .order_by("-created_at")
        )
...
class ContactPrayerIntentionsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        contact_id = self.kwargs.get("pk")
        user = self.request.user
        qs = PrayerIntention.objects.filter(contact_id=contact_id).select_related("contact")
        visible = get_visible_user_ids(user, request=self.request)
...
class MergeContactsView(APIView):
    ...
    def post(self, request):
        ...
        try:
            survivor_contact = Contact.objects.get(
                pk=ser.validated_data["survivor_id"], owner_id__in=visible
            )
            loser_contact = Contact.objects.get(
                pk=ser.validated_data["loser_id"], owner_id__in=visible
            )
        except Contact.DoesNotExist:
            return Response({"detail": "Contact not found."}, status=status.HTTP_404_NOT_FOUND)
        if survivor_contact.owner_id != loser_contact.owner_id:
            return Response(
                {"detail": "Contacts must belong to the same owner."},
        ...
        if request.user.role != "admin" and survivor_contact.owner_id != request.user.id:
            return Response(
                {"detail": "You do not have permission to merge these contacts."},
...
class DismissDuplicateView(APIView):
    ...
    def post(self, request):
        ...
        if request.user.role != "admin" and contact_a.owner_id != request.user.id:
            return Response(
                {"detail": "You do not have permission to dismiss these duplicates."},
                status=status.HTTP_403_FORBIDDEN,
            )
