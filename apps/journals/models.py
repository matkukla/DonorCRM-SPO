...
class Journal(TimeStampedModel):
    ...
    name = models.CharField(
        "name", max_length=255, help_text='Journal name (e.g., "Q1 2025 Campaign")'
    )
    ...
    deadline = models.DateField(
        "deadline", null=True, blank=True, help_text="Target completion date"
    )
...
class JournalContact(TimeStampedModel):
    ...
    journal = models.ForeignKey(
        "Journal", on_delete=models.CASCADE, related_name="journal_contacts", db_index=True
    )
...
class JournalStageEvent(TimeStampedModel):
    ...
    journal_contact = models.ForeignKey(
        "JournalContact", on_delete=models.CASCADE, related_name="stage_events", db_index=True
    )
    ...
    # Encrypted at rest: free-text donor-interaction notes are PII-adjacent
    # and not used in search/filter queries.
    notes = EncryptedTextField("notes", blank=True, help_text="Optional notes about this event")
...
class Decision(TimeStampedModel):
    ...
    journal_contact = models.ForeignKey(
        "JournalContact", on_delete=models.CASCADE, related_name="decisions", db_index=True
    )
    ...
    cadence = models.CharField(
        "cadence", max_length=20, choices=DecisionCadence.choices, default=DecisionCadence.MONTHLY
    )
...
class DecisionHistory(TimeStampedModel):
    ...
    decision = models.ForeignKey(
        "Decision", on_delete=models.CASCADE, related_name="history", db_index=True
    )

    changed_fields = models.JSONField(
        "changed fields", help_text="Mapping of field names to old values before change"
    )
...
class NextStep(TimeStampedModel):
    ...
    journal_contact = models.ForeignKey(
        "JournalContact", on_delete=models.CASCADE, related_name="next_steps", db_index=True
    )

    title = models.CharField(
        "title", max_length=255, help_text="Brief description of the next step"
    )

    notes = models.TextField("notes", blank=True, help_text="Optional additional details")

    due_date = models.DateField(
        "due date", null=True, blank=True, help_text="Optional target completion date"
    )

    completed = models.BooleanField("completed", default=False, db_index=True)

    completed_at = models.DateTimeField("completed at", null=True, blank=True)

    order = models.PositiveIntegerField(
        "display order", default=0, help_text="Order in checklist (lower = first)"
    )
