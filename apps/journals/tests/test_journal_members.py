...
class JournalContactTests(APITestCase):
    ...
    def test_add_contact_to_journal_success(self):
        ...
        data = {"journal": str(self.journal.id), "contact": str(self.contact_a1.id)}
    ...
    def test_add_multiple_contacts(self):
        ...
        response1 = self.client.post(
            url,
            {"journal": str(self.journal.id), "contact": str(self.contact_a1.id)},
            format="json",
        )
        ...
        response2 = self.client.post(
            url,
            {"journal": str(self.journal.id), "contact": str(self.contact_a2.id)},
            format="json",
        )
        ...
        response3 = self.client.post(
            url,
            {"journal": str(self.journal.id), "contact": str(self.contact_a3.id)},
            format="json",
        )
        self.assertEqual(response3.status_code, status.HTTP_201_CREATED)

        self.assertEqual(JournalContact.objects.filter(journal=self.journal).count(), 3)

    def test_remove_contact_from_journal(self):
        """Test removing a contact from a journal."""
        # Create membership
        jc = JournalContact.objects.create(journal=self.journal, contact=self.contact_a1)
    ...
    def test_duplicate_membership_returns_400(self):
        ...
        self.assertTrue("detail" in response2.data or "non_field_errors" in response2.data)
    ...
    def test_contact_in_multiple_journals(self):
        ...
        journal2 = Journal.objects.create(
            owner=self.user_a, name="Q2 2025 Campaign", goal_amount=60000.00
        )
        ...
        response2 = self.client.post(
            url, {"journal": str(journal2.id), "contact": str(self.contact_a1.id)}, format="json"
        )
        self.assertEqual(response2.status_code, status.HTTP_201_CREATED)

        # Verify contact is in both journals
        self.assertEqual(JournalContact.objects.filter(contact=self.contact_a1).count(), 2)
    ...
    def test_search_by_first_name(self):
        ...
        url = reverse("journals:journal-member-list")
        response = self.client.get(url, {"journal_id": str(self.journal.id), "search": "Alice"})

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 1)
        self.assertEqual(str(response.data["results"][0]["contact"]), str(self.contact_a1.id))

    def test_search_by_email(self):
        """Search memberships by exact contact email.

        email is encrypted at rest with a HMAC-SHA256 blind index for
        equality (apps.core.blind_index). Substring email search is no
        longer supported — see docs/security/data-classification.md.
        """
        # Add all contacts to journal
        JournalContact.objects.create(journal=self.journal, contact=self.contact_a1)
        JournalContact.objects.create(journal=self.journal, contact=self.contact_a2)
        ...
        response = self.client.get(
            url,
            {
                ...
                "search": "charlie@testdomain.com",  # exact match required
            },
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 1)
        self.assertEqual(str(response.data["results"][0]["contact"]), str(self.contact_a3.id))

    def test_partial_email_search_no_longer_matches(self):
        """Substring email search returns no rows after Phase 3B encryption."""
        JournalContact.objects.create(journal=self.journal, contact=self.contact_a3)
        ...
        response = self.client.get(
            ...
            {
                "journal_id": str(self.journal.id),
                "search": "testdomain",  # partial — must NOT match
            },
        )
    ...
    def test_search_case_insensitive(self):
        ...
        url = reverse("journals:journal-member-list")
        response = self.client.get(url, {"search": "ALICE"})  # Uppercase
    ...
    def test_filter_by_contact_status(self):
        ...
        url = reverse("journals:journal-member-list")
        response = self.client.get(
            url, {"journal_id": str(self.journal.id), "contact__status": "donor"}
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 1)
        self.assertEqual(str(response.data["results"][0]["contact"]), str(self.contact_a2.id))
        self.assertEqual(response.data["results"][0]["contact_status"], "donor")

    def test_filter_and_search_combined(self):
        """Test combining search and filter parameters."""
        # Add contacts
        JournalContact.objects.create(
            journal=self.journal, contact=self.contact_a1
        )  # Alice, prospect
        JournalContact.objects.create(journal=self.journal, contact=self.contact_a2)  # Bob, donor
        JournalContact.objects.create(
            journal=self.journal, contact=self.contact_a3
        )  # Charlie, asked
    ...
    def test_list_with_journal_id_filter(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 1)
        self.assertEqual(str(response.data["results"][0]["journal"]), str(self.journal.id))

        # Filter by second journal
        response = self.client.get(url, {"journal_id": str(journal2.id)})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 1)
        self.assertEqual(str(response.data["results"][0]["journal"]), str(journal2.id))
