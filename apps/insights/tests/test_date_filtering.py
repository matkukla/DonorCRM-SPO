...
class TestActivityHeatmap:
    """Tests for /api/v1/insights/admin/activity-heatmap/"""

    def test_admin_can_access(self, admin_client):
        """Verify admin can access activity heatmap endpoint."""
        client, admin_user = admin_client
        response = client.get('/api/v1/insights/admin/activity-heatmap/')
        assert response.status_code == status.HTTP_200_OK
        assert 'activities' in response.data

    def test_staff_cannot_access(self, authenticated_client):
        """Verify staff cannot access activity heatmap endpoint."""
        client, user = authenticated_client
        response = client.get('/api/v1/insights/admin/activity-heatmap/')
        assert response.status_code == status.HTTP_403_FORBIDDEN

    def test_returns_365_days_by_default(self, admin_client):
        """Verify heatmap returns approximately 365 days of data by default."""
        client, admin_user = admin_client
        response = client.get('/api/v1/insights/admin/activity-heatmap/')
        assert response.status_code == status.HTTP_200_OK
        # Should return 365-366 days depending on timing
        assert len(response.data['activities']) >= 365
        assert len(response.data['activities']) <= 366

    def test_returns_correct_structure(self, admin_client):
        """Verify each activity has date and count fields."""
        client, admin_user = admin_client
        response = client.get('/api/v1/insights/admin/activity-heatmap/')
        assert response.status_code == status.HTTP_200_OK
        ...
        assert isinstance(activity['count'], int)
    ...
    def test_aggregates_multiple_activity_types(self, admin_client):
        ...
        # Create activities on same day (today)
        ...
        # Query heatmap for today only to verify activities are counted
        today = timezone.now().date().isoformat()
        response = client.get(f'/api/v1/insights/admin/activity-heatmap/?date_from={today}')
        assert response.status_code == status.HTTP_200_OK

        # Should have at least one day (today)
        assert len(response.data['activities']) >= 1
        # Today should have at least 3 activities
        today_activity = response.data['activities'][0]
        assert today_activity['date'] == today
        assert today_activity['count'] >= 3

    def test_date_range_filters_activities(self, admin_client):
        """Verify date_from and date_to filter the activity range."""
        client, admin_user = admin_client
        date_from = (timezone.now() - timedelta(days=7)).date().isoformat()
        date_to = timezone.now().date().isoformat()
        response = client.get(f'/api/v1/insights/admin/activity-heatmap/?date_from={date_from}&date_to={date_to}')
        assert response.status_code == status.HTTP_200_OK
        # Should return approximately 7 days of data
        assert len(response.data['activities']) >= 7
        assert len(response.data['activities']) <= 8  # Allow for inclusive end
