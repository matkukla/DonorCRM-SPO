...
| R2 | Audit log erasable by an attacker with DB credentials (app-layer append-only only) | Low | Medium | High | **Deferred — accepted residual for the internal pilot.** Provisioning SQL + cutover runbook are staged for before external prospects ([dataaccesslog-role-lockdown.sql](sql/dataaccesslog-role-lockdown.sql), [db-role-cutover.md](db-role-cutover.md)). App stays on the owner role; the audit log remains append-only at the application layer. |
| R3 | Access token valid for up to ~15 min after logout (stateless JWT) | Low | Low–Med | Medium | **Accepted as-is.** Tokens live in `localStorage`, so XSS — not the window — is the dominant token-theft risk, and a strict CSP already mitigates that. Fully closing the window (a token-version denylist) would add complexity to the auth hot path for marginal gain. The real future lever is HttpOnly-cookie storage (blocked today by the cross-site `onrender` domains). Refresh tokens are revoked on logout/deactivation/password change. |
